...
Most founders, when they tell you their company is compounding, are hand-waving. They mean: things feel like they're getting better, the team has more momentum than they did three months ago, the next launch is going to be easier than the last one. It's a feeling. It's a vibe. It's the kind of thing that gets written into a pitch deck under "Why now" and is never, ever charted.
...
That's compounding. Not the metaphor. The actual phenomenon. And here's the thing nobody talks about: it doesn't usually look like anything. There's no chart. There's no measurement. There's no way to tell from outside whether a company is compounding or just running fast.

A caveat before we go further: this is one month of data, on seven moments. Four weeks of accelerating cadence is consistent with compounding, and it's also consistent with a learning curve on a new toolchain, with low-hanging fruit getting picked first, or with a productive stretch that hasn't ended yet. The chart doesn't tell you which. I'm calling it compounding because the structural reasons for the acceleration — substrate reuse, rule-citation densification, AI co-author retention — are visible in the artifacts and not just in the trend line. But the claim earns its weight by surviving more time. If the next four weeks show day-gaps reverting to 16+ days, the chart isn't compounding; it's a productive month that ended, and I'll republish it as such. *Section 10 lays out the falsification cases for this and several other claims explicitly.*

This essay is about why my company has the chart, what it took to build, and whether you can install one in yours.
...
One more disclosure before the rest: *this article was drafted by the AI co-author the article describes.* Claude — Anthropic's Claude Code, the same agent I run inside the system — produced most of the prose, working against an outline I structured and against the conventions and rules already encoded in the corpus. I edited, pushed back on framing, and kept what landed. If that bothers you, you've already learned something useful about whether the system would fit your company. If it doesn't, you've already understood the point.
...
If you're going to build a system that compounds, the first thing you need is a substrate to compound on. In my company that substrate is a decision corpus. It is not a journal.
...
Advisory rules decay. The text persists; the force does not. People forget. New tools skip them. Context shifts and the rule keeps existing in the rules file while losing its force in the operation. This isn't pessimism. It's the empirically observed half-life of any guidance that depends on a human remembering it under pressure. The rule that prevents you from sending an email containing a fabricated claim is not the rule. The check that refuses to send the email is the rule.
...
The seven rows are why there's a chart. The dates of those rows are: April 13, April 13, April 13, April 29, May 4, May 6, May 8. The day-gaps between them, in order, are zero, zero, sixteen, five, two, two. Three rules promoted on the same day in mid-April, then a pause of more than two weeks, then a steadily shortening cadence over the past ten days.

![Rule promotions to load-bearing enforcement: 0 → 0 → 16 → 5 → 2 → 2 days]({static}/images/compounding-charted/02-lb-promotion-cadence.png)

The acceleration is the system itself getting cheaper to extend. Each new enforcement reuses substrate from the previous. The first promotion required building a verification module from scratch: how to extract claims from prose, how to canonicalize them for lookup, how to fail closed without breaking the developer flow. The second and third promotions plugged into that same module. The fourth promotion required different substrate (a content scanner for external markdown files); the fifth, sixth, and seventh plugged into that. By the time you get to the seventh, the marginal cost of "wire this rule into the substrate" is a couple of hours.

This is one specific kind of compounding worth distinguishing from the others the article points at. The cadence-acceleration shown in the chart is *substrate reuse*: each gate makes the next gate cheaper to wire. That's a learning-curve dynamic with structural reasons behind it. It's distinct from *corpus accumulation* (linear growth in the volume of decisions retained, not exponential), and distinct again from *rule-citation densification* (network-effect-style interconnection growth in the rule layer over time). All three feed each other in practice, but they're different phenomena. Lumping them under one word makes the system sound more unified than it is. The cadence chart speaks to the first; only the third has the network-effect property that "compounding" usually implies in its strongest sense.

The cadence chart is the most concrete claim of compounding I know how to make. Most operating systems don't have this chart because most operating systems don't have a load-bearing-rules table because most rule layers don't distinguish between advisory and enforcement. The distinction is the substance. If you can't say which of your rules are enforced and which are reminders, you're working in a system whose half-life you can't measure.
...
Most founder operating systems are paper. The decision log lives in Notion or a Google Doc. The rules live in a wiki. The conventions live in someone's head. None of those formats are queryable by anything other than a human, and the human only queries them when they remember to. The corpus exists, but it's inert.
...
The second was the cadence acceleration in the load-bearing rule promotions, which I've already shown you in chart two. I'd known the rule promotions were happening; I hadn't seen them as an accelerating cadence until I sorted them by date and computed the gaps.
...
Six structural patterns. None of them visible from reading individual decisions in sequence. None of them discoverable in a Notion doc. None requiring infrastructure I didn't already have. About an hour of compute and a Python script that read the JSON Lines file the corpus rebuilds itself into every time a decision-log file is edited.
...
This is the loop that closes the corpus on itself. Without it, the corpus is a record. With it, the corpus becomes a teacher.
...
A reasonable first version takes one to two weeks of engineering. Subsequent rules plug into the substrate cheaply. This is where the cadence acceleration in chart 2 comes from.
